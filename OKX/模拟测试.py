...
class VirtualTradingSystem:
    def __init__(self, initial_balance=100.0):
        ...
        self.virtual_positions = {}  # symbol -> position info
        self.trade_records = []
        self.record_file = "交易详情.txt"
        
        # 初始化记录文件
        with open(self.record_file, 'w', encoding='utf-8') as f:
            f.write(f"虚拟交易记录 - 初始余额: ${initial_balance}\n")
            f.write(f"开始时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write("=" * 60 + "\n")
    
    def record_trade(self, symbol_config, signal_data, price_data, is_open=True, pnl_usdt=0, pnl_percent=0):
        """记录交易到文件"""
        timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        symbol_name = symbol_config['display_name']
        
        record = f"\n{'-'*50}\n"
        record += f"时间: {timestamp}\n"
        record += f"币种: {symbol_name}\n"
        
        if is_open:
            record += f"操作: 开仓 ({signal_data['signal']})\n"
            record += f"开仓价格: ${price_data['price']:,.2f}\n"
            record += f"止损价格: ${signal_data.get('stop_loss', 0):,.2f}\n"
            record += f"止盈价格: ${signal_data.get('take_profit', 0):,.2f}\n"
            record += f"仓位大小: {signal_data['position_size']:.2f}张\n"
            record += f"杠杆倍数: {signal_data['leverage']}x\n"
            record += f"风险比例: {signal_data['risk_percentage']:.1f}%\n"
        else:
            record += f"操作: 平仓\n"
            record += f"平仓价格: ${price_data['price']:,.2f}\n"
            record += f"盈亏金额: ${pnl_usdt:+,.2f} USDT\n"
            record += f"盈亏比例: {pnl_percent:+.2f}%\n"
        
        record += f"虚拟余额: ${self.virtual_balance:,.2f}\n"
        record += f"{'-'*50}"
        ...
        print(f"📝 交易记录已保存到 {self.record_file}")
    
    def calculate_position_cost(self, position_size, contract_size, price, leverage):
        """计算仓位成本"""
        position_value = position_size * contract_size * price
        required_margin = position_value / leverage
        return required_margin
    
    def open_position(self, symbol_config, signal_data, price_data):
        """虚拟开仓"""
        ...
        position_cost = self.calculate_position_cost(
            ...
            symbol_config['contract_size'],
            price_data['price'],
            signal_data['leverage']
        )
        
        # 检查余额是否足够
        if position_cost > self.virtual_balance:
            print(f"💰 虚拟余额不足: 需要${position_cost:.2f}, 当前${self.virtual_balance:.2f}")
            return False
        
        # 扣除保证金
        self.virtual_balance -= position_cost
        ...
        # 记录交易
        self.record_trade(symbol_config, signal_data, price_data, is_open=True)
        
        print(f"✅ 虚拟开仓成功: {symbol_config['display_name']} "
              f"{signal_data['signal']} {signal_data['position_size']:.2f}张 "
              f"价格${price_data['price']:.2f} 余额${self.virtual_balance:.2f}")
        
        return True
    
    def close_position(self, symbol_config, price_data, reason="手动平仓"):
        """虚拟平仓"""
        symbol = symbol_config['symbol']
        
        if symbol not in self.virtual_positions:
            print(f"⚠️  没有 {symbol_config['display_name']} 的持仓")
            return False
        ...
        # 计算盈亏金额（简化计算）
        position_value = position['size'] * symbol_config['contract_size'] * entry_price
        pnl_usdt = position_value * (pnl_percent / 100)
        
        # 返还保证金并添加盈亏
        self.virtual_balance += position['cost'] + pnl_usdt
        
        # 记录交易
        self.record_trade(symbol_config, {}, price_data, is_open=False, 
                         pnl_usdt=pnl_usdt, pnl_percent=pnl_percent)
        
        # 移除持仓
        del self.virtual_positions[symbol]
        
        print(f"✅ 虚拟平仓成功: {symbol_config['display_name']} "
              f"{position['side']}仓 {position['size']:.2f}张 "
              f"开${entry_price:.2f} 平${exit_price:.2f} "
              f"盈亏{pnl_percent:+.2f}% (${pnl_usdt:+.2f}) "
              f"余额${self.virtual_balance:.2f}")
        
        return True

# 初始化虚拟交易系统
virtual_trading = VirtualTradingSystem(initial_balance=100.0)
...
def get_current_position(symbol):
    ...
    try:
        # 检查虚拟持仓
        if symbol in virtual_trading.virtual_positions:
            pos = virtual_trading.virtual_positions[symbol]
            return {
                'side': 'long' if pos['side'] == 'buy' else 'short',
                'size': pos['size'],
                'entry_price': pos['entry_price'],
                'unrealized_pnl': 0,  # 虚拟交易暂不计算浮动盈亏
...
def analyze_with_deepseek_full_control(price_data, symbol_config, account_info):
    ...
    position_text = "无持仓" if not current_position else f"{current_position['side']}仓, 数量: {current_position['size']:.2f}张"
...
def get_account_info(symbol_config, current_price):
    ...
    try:
        # 使用虚拟账户信息
        total_balance = virtual_trading.virtual_balance
        ...
        print(f"❌ 获取账户信息失败: {e}")
        return {
            'total': virtual_trading.virtual_balance,
            'free': virtual_trading.virtual_balance,
            'position_value': 0,
            'exposure_pct': 0,
            'current_price': current_price
        }
...
def execute_full_control_trade(symbol_config, signal_data, price_data, account_info):
    ...
    try:
        ...
        if signal_data['signal'] == 'BUY':
            execute_virtual_buy_trade(symbol_config, signal_data, price_data)
        elif signal_data['signal'] == 'SELL':
            execute_virtual_sell_trade(symbol_config, signal_data, price_data)
        elif signal_data['signal'] == 'HOLD':
            print("🤚 观望策略，不执行交易")
        
        print(f"✅ {display_name} 虚拟交易执行完成")
        time.sleep(2)
        
    except Exception as e:
        print(f"❌ {display_name} 虚拟交易执行失败: {e}")

def execute_virtual_buy_trade(symbol_config, signal_data, price_data):
    """执行虚拟买入交易"""
    symbol = symbol_config['symbol']
    ...
    if current_position and current_position['side'] == 'short':
        # 平空仓
        virtual_trading.close_position(symbol_config, price_data, "反向开仓平仓")
        time.sleep(0.5)
    
    # 开多仓
    virtual_trading.open_position(symbol_config, signal_data, price_data)

def execute_virtual_sell_trade(symbol_config, signal_data, price_data):
    """执行虚拟卖出交易"""
    symbol = symbol_config['symbol']
    ...
    # 开空仓
    signal_data_for_sell = signal_data.copy()
    signal_data_for_sell['signal'] = 'SELL'  # 确保信号为SELL
    virtual_trading.open_position(symbol_config, signal_data_for_sell, price_data)
...
def multi_symbol_full_control_bot():
    ...
    print(f"💰 虚拟账户余额: ${virtual_trading.virtual_balance:.2f}")
    print(f"{'='*60}")
...
def main():
    ...
    print(f"💰 虚拟账户初始金额: $100 USDT")
    print(f"📊 交易记录文件: {virtual_trading.record_file}")
    print(f"支持币种: {len(TRADE_SYMBOLS)}个")
    print("=" * 60)
    
    print("当前为虚拟交易测试模式，使用真实市场数据")
    print("所有交易均为虚拟执行，不会真实下单")
    ...
    print(f"💰 当前虚拟余额: ${virtual_trading.virtual_balance:.2f}")
